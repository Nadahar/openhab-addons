...
public class Shelly1CoapHandler implements Shelly1CoapListener {
    ...
    private final ShellyRuntimeConfiguration runtimeConfig;
    ...
    public Shelly1CoapHandler(ShellyThingInterface thingHandler, String thingName,
            ShellyRuntimeConfiguration runtimeConfig, Shelly1CoapServer coapServer) {
    ...
    public synchronized void start() throws ShellyApiException {
        try {
            ...
            coapServer.start(runtimeConfig.getLocalIp(), coiotPort, this);
            statusClient = new CoapClient(completeUrl(runtimeConfig.getDeviceIp(), coiotPort, COLOIT_URI_DEVSTATUS))
            ...
            throw new ShellyApiException("Unknown Host: " + runtimeConfig.getDeviceIp(), e);
        }
    }
    ...
    public void processResponse(@Nullable Response response) {
        ...
        boolean match = ip.contains("/" + runtimeConfig.getDeviceIp() + ":");
        if (!match) {
        ...
        if (!updatesRequested) {
            ...
            reqStatus = sendRequest(reqStatus, runtimeConfig.getDeviceIp(), COLOIT_URI_DEVSTATUS, Type.NON);
    ...
    private void discover() {
        ...
        reqDescription = sendRequest(reqDescription, runtimeConfig.getDeviceIp(), COLOIT_URI_DEVDESC, Type.CON);
    }
