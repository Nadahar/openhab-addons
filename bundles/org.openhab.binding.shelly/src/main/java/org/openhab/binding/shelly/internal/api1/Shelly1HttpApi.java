...
public class Shelly1HttpApi extends ShellyHttpClient implements ShellyApiInterface {
     ...
     */
    public Shelly1HttpApi(String thingName, ShellyThingConfiguration config, ShellyRuntimeConfiguration runtimeConfig,
    ...
    private void setSensorEventUrls() throws ShellyApiException, ShellyApiException {
        if (profile.isSensor) {
            ...
            setEventUrl(!config.getEventsCoIoT() && config.getEventsSensorReport(), SHELLY_EVENT_SENSORREPORT,
                    SHELLY_EVENT_DARK, SHELLY_EVENT_TWILIGHT, SHELLY_EVENT_FLOOD_DETECTED, SHELLY_EVENT_FLOOD_GONE,
                    SHELLY_EVENT_OPEN, SHELLY_EVENT_CLOSE, SHELLY_EVENT_VIBRATION, SHELLY_EVENT_ALARM_MILD,
                    SHELLY_EVENT_ALARM_HEAVY, SHELLY_EVENT_ALARM_OFF, SHELLY_EVENT_TEMP_OVER, SHELLY_EVENT_TEMP_UNDER);
        }
    }
    ...
    private void setEventUrls(Integer index) throws ShellyApiException {
        ...
        if (profile.isRoller) {
            ...
            // 2 set of URLs
            setEventUrl(EVENT_TYPE_LIGHT, index, !eventsCoIoT && config.getEventsButton(), SHELLY_EVENT_BTN1_ON,
                    SHELLY_EVENT_BTN1_OFF, SHELLY_EVENT_BTN2_ON, SHELLY_EVENT_BTN2_OFF);
            setEventUrl(EVENT_TYPE_LIGHT, index, !eventsCoIoT && config.getEventsPush(), SHELLY_EVENT_SHORTPUSH1,
                    SHELLY_EVENT_LONGPUSH1, SHELLY_EVENT_SHORTPUSH2, SHELLY_EVENT_LONGPUSH2);

            // Relay output
            setEventUrl(EVENT_TYPE_LIGHT, index, !eventsCoIoT && config.getEventsSwitch(), SHELLY_EVENT_OUT_ON,
                    SHELLY_EVENT_OUT_OFF);
        } else if (profile.hasRelays) {
            // Standard relays: btn_xxx, out_xxx, short/longpush URLs
            setEventUrl(EVENT_TYPE_RELAY, index, !eventsCoIoT && config.getEventsButton(), SHELLY_EVENT_BTN_ON,
                    SHELLY_EVENT_BTN_OFF);
            setEventUrl(EVENT_TYPE_RELAY, index, !eventsCoIoT && config.getEventsPush(), SHELLY_EVENT_SHORTPUSH,
                    SHELLY_EVENT_LONGPUSH);
            setEventUrl(EVENT_TYPE_RELAY, index, !eventsCoIoT && config.getEventsSwitch(), SHELLY_EVENT_OUT_ON,
                    SHELLY_EVENT_OUT_OFF);
        } else if (profile.isLight) {
            // Duo, Bulb
            setEventUrl(EVENT_TYPE_LIGHT, index, !eventsCoIoT && config.getEventsSwitch(), SHELLY_EVENT_OUT_ON,
