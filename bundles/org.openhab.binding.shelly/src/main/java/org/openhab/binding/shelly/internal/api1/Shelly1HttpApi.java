...
public class Shelly1HttpApi extends ShellyHttpClient implements ShellyApiInterface {
    ...
    private void setEventUrls(Integer index) throws ShellyApiException {
        ...
        if (profile.isRoller) {
            ...
            // 2 set of URLs
            setEventUrl(EVENT_TYPE_LIGHT, index, !eventsCoIoT && config.getEventsButton(), SHELLY_EVENT_BTN1_ON, SHELLY_EVENT_BTN1_OFF,
            ...
            // Relay output
            setEventUrl(EVENT_TYPE_LIGHT, index, !eventsCoIoT && config.getEventsSwitch(), SHELLY_EVENT_OUT_ON, SHELLY_EVENT_OUT_OFF);
        } else if (profile.hasRelays) {
            // Standard relays: btn_xxx, out_xxx, short/longpush URLs
            setEventUrl(EVENT_TYPE_RELAY, index, !eventsCoIoT && config.getEventsButton(), SHELLY_EVENT_BTN_ON, SHELLY_EVENT_BTN_OFF);
            setEventUrl(EVENT_TYPE_RELAY, index, !eventsCoIoT && config.getEventsPush(), SHELLY_EVENT_SHORTPUSH, SHELLY_EVENT_LONGPUSH);
            setEventUrl(EVENT_TYPE_RELAY, index, !eventsCoIoT && config.getEventsSwitch(), SHELLY_EVENT_OUT_ON, SHELLY_EVENT_OUT_OFF);
        } else if (profile.isLight) {
            // Duo, Bulb
            setEventUrl(EVENT_TYPE_LIGHT, index, !eventsCoIoT && config.getEventsSwitch(), SHELLY_EVENT_OUT_ON, SHELLY_EVENT_OUT_OFF);
        }
